...
x0 = mod( sha_sum/(256*32) + bin2dec(num2str(x(1:8)))/256,1);
miu = mod( sha_sum/(256*32) + bin2dec(num2str(x(10:18)))/256, 0.1) + 3.9;
miu1 = mod( sha_sum/(256*32) + bin2dec(num2str(x(20:28)))/256,1) * 10 + 30;
miu2 = mod( sha_sum/(256*32) + bin2dec(num2str(x(30:38)))/256,1) * 10 + 30;

dnkey = [x0,miu,miu1,miu2];
...
y = [];
y(:,1) = mod(init(1:len/2)+init(len/2+1:len) ,1);

[~,aa] = sort(init(1:len/2));
[~,bb] = sort(init(len/2+1:len));

for i=1:L+100
    y(:,i+1)=mod(miu1./(y(:,i).*sin(miu2./ (y(aa,i).*y(bb,i)))),1);
end
